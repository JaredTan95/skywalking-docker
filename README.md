...
- ```Elasticsearch安装(如果已安装可跳过此步骤)，版本要求5.x：docker run -p 9200:9200 -p 9300:9300 -e cluster.name=elasticsearch -e xpack.security.enabled=false -e node.name=sky-es-docker-node01 -d docker.elastic.co/elasticsearch/elasticsearch:5.6.10```
